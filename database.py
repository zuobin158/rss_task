...
from settings import DB_CNF

debug = getattr(argparse, 'debug')
...
class Mysql(object):
    ...
    def do_execute(self, sql, values):
...
def trans(obj):
    '''
    事务修饰器，被该修饰器修饰的方法中,所有数据库
    增删改操作均在一个事务中,要么全部成功，要么全部失败。
    Args:
        obj:model类，其目的在于指定一个db，即该db下所有表的操作均在一个事务中。
    '''
    def wrapper(function):
        def transaction_wrapper_func(*args, **kwargs):
            #验证obj是否继承自Model类
            if not issubclass(obj, Model):
                raise Exception('model is a not valid type!')

            with obj(ismaster=True).transaction():
                function(*args, **kwargs)
        return transaction_wrapper_func
    return wrapper
